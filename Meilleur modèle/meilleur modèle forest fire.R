...
ts_forest_fire_2022 <- ts(ts_forest_fire, start = c(2000, 1), end = c(2022, 12), frequency = 12)
...
summary(regarima_x13(ts_forest_fire_2022, spec ="RG5c"))
...
pred_naive <- naive(ts_forest_fire_2022, h=12)
show(pred_naive)
plot(pred_naive)

####  StructTS ----

fitsts = StructTS(ts_forest_fire_2022)
...
decomp = stl(ts_forest_fire_2022, s.window="periodic")
# show(decomp)
plot(decomp)

fitstl = stlm(ts_forest_fire_2022)
...
mysax13 <- x13(ts_forest_fire_2022, myspec)
...
fit_hw <- HoltWinters(ts_forest_fire_2022, seasonal = "add")
fit_hw
...
n <- length(ts_forest_fire_2022)  # nombre d'observations
...
fitets <- ets(ts_forest_fire_2022, ic = "aic") # pour avoir le meilleur AIC
show(fitets)
plot(fitets)
...
fit_tbats <- tbats(ts_forest_fire_2022)
show(fit_tbats)
plot(fit_tbats)
...
fit_ADAM_ETS <- auto.adam(ts_forest_fire_2022, model = "NAA", lags = c(1, 12), select = TRUE)
fit_ADAM_ETS
...
fitadam3 <- auto.adam(ts_forest_fire_2022, model = "NAA", lags=c(1,12), orders=list(ar=c(3,3), i=(2), ma=c(3,3), select=TRUE))
...
fit_SSARIMA <- auto.ssarima(ts_forest_fire_2022, lags=c(1,12), orders=list(ar=c(3,3), i=(2), ma=c(3,3), select=TRUE))
fit_SSARIMA
...
fit_sarima <- auto.arima(ts_forest_fire_2022, seasonal = TRUE)
...
ts_forest_fire_2023 <- ts(ts_forest_fire_2022, 
...
ts_forest_fire_2023_df <- data.frame(
  date = as.Date(time(ts_forest_fire_2023)),
  value = as.numeric(ts_forest_fire_2023),
  model = "Observed Data"
)
...
ggplot() +
  ...
  geom_line(data = ts_forest_fire_2023_df, aes(x = date, y = value),
...
actual_values <- ts_forest_fire_2023
